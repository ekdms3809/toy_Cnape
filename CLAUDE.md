# CLAUDE.md — 이 레포에서 AI가 지킬 규칙

1인 프로젝트. 코드는 AI(Claude)와 함께 짜지만 리뷰·머지·최종 판단은 전부 나(레포 주인)가 한다.

---

## 항상 강제

- 코드 작성 전 변경 범위·방법을 한국어로 제시 → 동의 후 작성
- 새 기능/버그수정에는 pytest 테스트 동반 (예외는 명시 승인 시만)
- PR 전 lint + pytest 로컬 통과 확인 — 통과 전 PR 생성 금지
- 커밋·PR·이슈 메시지는 한국어
- 스캐너는 **읽기 전용 API만** 호출 — 수정은 dry-run 스크립트 제안까지만, 실행 금지
...
- 머지된 PR은 다시 건드리지 않음 — 피드백은 새 PR 대신 같은 브랜치에 커밋 추가
...
- AI 셀프 머지 — PR 생성까지만, 머지 버튼은 반드시 내가 누름
...
| 브랜치 | 대상 | 상태 |
|--------|------|------|
| `feat/ec2-scanner` | EC2 보안그룹 오픈포트 탐지 | 구현 완료, main 반영 대기 |
| `feat/s3-scanner` | S3 오설정 감지 | 다음 착수 |
| `feat/iam-scanner` | IAM 과잉권한·미사용키 | 정책 시맨틱 재설계 필요 |
